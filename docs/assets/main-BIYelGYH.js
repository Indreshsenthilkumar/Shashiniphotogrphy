import"./style-Jv1gWvqc.js";const p="http://127.0.0.1:5001/api",b="https://script.google.com/macros/s/AKfycbwL3REAruQwojPkDr1AhCOzCGb18pTT7o1S0qX2nhv6rwEObuyYYJxzdiL6zivQAxG9/exec",L="https://script.google.com/macros/s/AKfycbxu6hp66vbNm1k6QAoy-or27HxtZ1nC8aD79rdUIaHsyGxXNkU7rSBYHKlS1vAErn8n/exec",d=document.getElementById("app"),y=document.querySelector(".profile-btn");function u(e){return e?e.startsWith("http")||e.startsWith("data:")||e.startsWith("./")||e.startsWith("../")?e:e.startsWith("/")?p+e:p+"/"+e:""}window.onerror=function(e){return console.error("[App Shield]",e),!0};window.onunhandledrejection=function(e){console.error("[App Shield] Rejection",e.reason)};async function k(e,i={},o=1e4){const s=new AbortController,a=setTimeout(()=>s.abort(),o);try{const l=await fetch(e,{...i,signal:s.signal});if(clearTimeout(a),!l.ok)throw new Error(`HTTP ${l.status}`);return await l.json()}catch(l){return clearTimeout(a),console.warn(`[SecureFetch] Error: ${e}`,l.message),null}}let t={user:JSON.parse(localStorage.getItem("user"))||null,view:"home",vaults:[],currentVault:null,photos:[],selectedPhotos:new Set,cms:{items:[],hero:{slides:[],interval:5}},messages:[]};async function S(){document.getElementById("global-loader")||document.body.insertAdjacentHTML("beforeend",`
        ...
        `,document.body.appendChild(e),setTimeout(()=>document.getElementById("admin-user").focus(),100),document.getElementById("admin-submit").onclick=()=>{const i=document.getElementById("admin-user").value,o=document.getElementById("admin-pass").value;i==="admin123"&&o==="098765"?window.location.href="/admin.html":alert("Invalid Credentials")}},y)if(t.user){const e=t.user.name||"User",i=e.charAt(0).toUpperCase();y.innerHTML=`
            ...
            `;switch(t.view){case"home":$();break;case"gallery":M();break;case"bookings":A();break;case"vault":z();break;case"messages":I();break;case"profile":renderProfileView();break}}function $(){d.innerHTML=`
...
Please contact us for assistance.`),i.disabled=!1,i.innerHTML=`✓ FINALIZE & SUBMIT (${t.selectedPhotos.size})`}}window.renderProfileView=async function(){if(!t.user)return showLoginModal();d.innerHTML='<div class="container" style="text-align:center; padding: 100px;"><p>Loading your profile...</p></div>';try{const[e,i]=await Promise.all([fetch(`${p}/vaults?mobile=${t.user.mobile}`).then(o=>o.json()),fetch(`${b}?action=getProfile&mobile=${t.user.mobile}`).then(o=>o.json())]);i&&i.found?(t.user.name=i.name||t.user.name,t.user.email=i.email||t.user.email,t.user.loginCount=i.loginCount,t.user.lastLogin=i.lastLogin,t.user.collectionSummary=i.collectionSummary):t.user.collectionSummary=e.length+" Sessions",d.innerHTML=`
        ...
        `}catch(e){if(console.error("Profile load error:",e),!t.user)return showLoginModal();d.innerHTML=`
            ...
            </div>`}};async function I(){d.innerHTML=`
