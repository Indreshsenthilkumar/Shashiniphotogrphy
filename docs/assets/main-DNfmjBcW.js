import"./style-BqiAwLdI.js";const p="http://127.0.0.1:5001/api",b="https://script.google.com/macros/s/AKfycbwL3REAruQwojPkDr1AhCOzCGb18pTT7o1S0qX2nhv6rwEObuyYYJxzdiL6zivQAxG9/exec",I="https://script.google.com/macros/s/AKfycbxu6hp66vbNm1k6QAoy-or27HxtZ1nC8aD79rdUIaHsyGxXNkU7rSBYHKlS1vAErn8n/exec",d=document.getElementById("app"),v=document.querySelector(".profile-btn");function u(e){return e?e.startsWith("http")||e.startsWith("data:")||e.startsWith("./")||e.startsWith("../")?e:e.startsWith("/")?p+e:p+"/"+e:""}window.onerror=function(e){return console.error("[App Shield]",e),!0};window.onunhandledrejection=function(e){console.error("[App Shield] Rejection",e.reason)};async function k(e,i={},o=1e4){const s=new AbortController,a=setTimeout(()=>s.abort(),o);try{const l=await fetch(e,{...i,signal:s.signal});if(clearTimeout(a),!l.ok)throw new Error(`HTTP ${l.status}`);return await l.json()}catch(l){return clearTimeout(a),console.warn(`[SecureFetch] Error: ${e}`,l.message),null}}let t={user:JSON.parse(localStorage.getItem("user"))||null,view:"home",vaults:[],currentVault:null,photos:[],selectedPhotos:new Set,cms:{items:[],hero:{slides:[],interval:5}},messages:[]};async function S(){document.getElementById("global-loader")||document.body.insertAdjacentHTML("beforeend",`
        ...
        `);const e=document.getElementById("global-loader");window.showLoader=()=>e.classList.add("active"),window.hideLoader=()=>e.classList.remove("active"),window.state=t,window.render=m,window.onscroll=()=>{document.body.classList.toggle("scrolled",window.scrollY>50)},t.view==="home"&&document.body.classList.add("home-view"),window.setView=n=>{t.view!==n&&(t.view=n,window.innerWidth<768&&(t.viewMode="cards"),document.body.classList.toggle("home-view",n==="home"),document.querySelectorAll(".nav-link").forEach(r=>r.classList.toggle("active",r.dataset.view===n)),document.querySelectorAll(".mobile-nav-link").forEach(r=>r.classList.toggle("active",r.dataset.view===n)),m(),x().then(()=>{(n==="home"||n==="gallery")&&m()}))},T(),C(),window.showLoader();const i=x().catch(n=>console.warn("Background fetch failed",n)),o=new Promise(n=>setTimeout(n,1500));await Promise.race([i,o]),window.hideLoader(),i.finally(()=>m());const s=document.getElementById("hamburger-btn"),a=document.getElementById("close-menu-btn"),l=document.getElementById("mobile-menu");s&&l&&(s.onclick=()=>l.classList.add("active")),a&&l&&(a.onclick=()=>l.classList.remove("active")),v&&(window.location.hostname.includes("localhost")||window.location.hostname.includes("127.0.0.1")?v.onclick=()=>{t.user?(t.view="profile",m()):window.showLoginModal()}:v.style.display="none"),window.showLoginModal=()=>{const n=document.createElement("div");n.className="modal-overlay",n.innerHTML=`
        ...
        `,document.body.appendChild(n),setTimeout(()=>document.getElementById("login-mobile").focus(),100),document.getElementById("login-submit").onclick=async()=>{const r=document.getElementById("login-mobile").value,c=document.getElementById("login-name").value;if(r.length!==10)return alert("Please enter a valid 10-digit mobile number.");if(!c)return alert("Please enter your name.");window.showLoader();const g={mobile:r,name:c};localStorage.setItem("user",JSON.stringify(g)),t.user=g;try{await fetch(b,{method:"POST",mode:"no-cors",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"logLogin",mobile:r,name:c})})}catch(f){console.error("Cloud log failed:",f)}window.hideLoader(),n.remove(),m(),t.view==="vault"||t.view==="profile"?setView(t.view):alert("Welcome, "+c+"!")}},window.logout=()=>{confirm("Are you sure you want to logout?")&&(localStorage.removeItem("user"),t.user=null,t.vaults=[],setView("home"))},document.querySelectorAll(".mobile-nav-link").forEach(n=>{n.onclick=r=>{r.preventDefault();const c=n.dataset.view;c==="profile"?renderProfileView():setView(c),l.classList.remove("active")}}),m(),setInterval(async()=>{(t.view==="home"||t.view==="gallery")&&(await x(),m())},6e4)}async function x(){try{const e=await fetch(I).then(i=>i.json());e&&e.items?t.cms=e:t.cms={items:[],hero:{slides:[],interval:5}}}catch(e){if(console.error("[CMS Sync] Failed to fetch from Google Sheet:",e),window.location.hostname.includes("localhost")||window.location.hostname.includes("127.0.0.1")){const o=await k(`${p}/cms`);o&&(t.cms=o)}}}function T(){localStorage.getItem("theme")==="dark"&&document.body.classList.add("dark-mode"),window.toggleTheme=()=>{document.body.classList.toggle("dark-mode");const i=document.body.classList.contains("dark-mode")?"dark":"light";localStorage.setItem("theme",i)}}function C(){document.querySelectorAll(".nav-link, .mobile-nav-link").forEach(e=>{e.addEventListener("click",i=>{i.preventDefault();const o=e.getAttribute("href");if(o&&o.startsWith("#")){const a=o.substring(1);if(t.view!=="home")setView("home"),setTimeout(()=>{const n=document.getElementById(a);n&&n.scrollIntoView({behavior:"smooth"})},100);else{const n=document.getElementById(a);n&&n.scrollIntoView({behavior:"smooth"})}const l=document.getElementById("mobile-menu");l&&l.classList.remove("active");return}const s=e.dataset.view;if(s){if(s==="vault"&&!t.user)return showLoginModal();if(t.view!==s){setView(s);const a=document.getElementById("mobile-menu");a&&a.classList.remove("active")}}})})}function m(){if(window.scrollTo(0,0),document.body.classList.toggle("home-view",t.view==="home"),window.showAdminLoginModal=()=>{const e=document.createElement("div");e.className="modal-overlay",e.innerHTML=`
        ...
        `,document.body.appendChild(e),setTimeout(()=>document.getElementById("admin-user").focus(),100),document.getElementById("admin-submit").onclick=()=>{const i=document.getElementById("admin-user").value,o=document.getElementById("admin-pass").value;i==="admin123"&&o==="098765"?window.location.href="/admin.html":alert("Invalid Credentials")}},v)if(t.user){const e=t.user.name||"User",i=e.charAt(0).toUpperCase();v.innerHTML=`
            ...
            `;switch(t.view){case"home":$();break;case"gallery":M();break;case"bookings":A();break;case"vault":z();break;case"messages":L();break;case"profile":renderProfileView();break}}function $(){d.innerHTML=`
        ...
        <section class="new-hero fade-in">
            ...
            <div id="hero-media-container" style="position: absolute; inset:0; z-index:1;">
                ${t.cms.hero.slides.length>0?t.cms.hero.slides[0].type==="video"?`<video src="${u(t.cms.hero.slides[0].url)}" class="hero-slide-item" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover; opacity:1; z-index:1;" muted loop playsinline autoplay></video>`:`<img src="${u(t.cms.hero.slides[0].url)}" class="hero-slide-item" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover; opacity:1; z-index:1;">`:`
    ...
    `,t.view==="home"&&(document.body.classList.add("home-view"),V())}function P(){const e=i=>"";return`
        ...
        <div class="exhibition-grid">
            ...
            `:t.cms.items.slice(0,4).map((i,o)=>{var r;const s=i.url&&(i.url.startsWith("data:video/")||i.url.endsWith(".mp4")),a=i.url&&(i.url.includes("youtube.com")||i.url.includes("youtu.be")),l=e();let n="";return a?n=`<iframe src="https://www.youtube.com/embed/${(r=i.url.match(/(?:https?:\/{2})?(?:w{3}\.)?youtu(?:be)?\.(?:com|be)(?:\/watch\?v=|\/)([^\s&]+)/))==null?void 0:r[1]}" 
                            style="width: 100%; height: 100%; border: none;" 
                            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" 
                            allowfullscreen></iframe>`:s?n=`<video src="${u(i.url)}" muted loop playsinline onmouseover="this.play()" onmouseout="this.pause()"></video>`:n=`<img src="${u(i.url)}" alt="${i.title||"Exhibition Image"}">`,`
                    <div class="exhibition-item ${l} fade-in-up" 
                         style="animation-delay: ${.2+.1*o}s; cursor: pointer;" onclick="setView('gallery')">
                         ${n}
                        <div class="exhibition-overlay">
                            <span class="exhibition-tag">MASTERPIECE ${o+1}</span>
                            <h4 class="exhibition-title">${i.title||"Untitled Work"}</h4>
                        </div>
                    </div>
                `}).join("")}
        </div>
    ...
    `}function M(){d.innerHTML=`
        <div class="container slide-up" style="margin-top: 60px;">
            ...
            <div class="photo-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 30px;">
                ...
                `:t.cms.items.map(e=>{var a;const i=e.url&&(e.url.startsWith("data:video/")||e.url.endsWith(".mp4")),o=e.url&&(e.url.includes("youtube.com")||e.url.includes("youtu.be"));let s="";return o?s=`<iframe src="https://www.youtube.com/embed/${(a=e.url.match(/(?:https?:\/{2})?(?:w{3}\.)?youtu(?:be)?\.(?:com|be)(?:\/watch\?v=|\/)([^\s&]+)/))==null?void 0:a[1]}" 
                            style="width: 100%; height: 100%; border: none;" 
                            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" 
                            allowfullscreen></iframe>`:i?s=`<video src="${u(e.url)}" style="width:100%; height:100%; object-fit:cover;" muted controls playsinline></video>`:s=`<img src="${u(e.url)}" alt="${e.title}" loading="lazy" style="width:100%; height:100%; object-fit:cover;">`,`
        ...
        `,function(e,i,o){let s=function(l,n){l.q.push(n)},a=e.document;e.Cal=e.Cal||function(){let l=e.Cal,n=arguments;if(l.loaded||(l.ns={},l.q=l.q||[],a.head.appendChild(a.createElement("script")).src=i,l.loaded=!0),n[0]===o){const r=function(){s(r,arguments)},c=n[1];r.q=r.q||[],typeof c=="string"?(l.ns[c]=l.ns[c]||r,s(l.ns[c],n),s(l,["initNamespace",c])):s(l,n);return}s(l,n)}}(window,"https://app.cal.com/embed/embed.js","init"),Cal("init","shashini-studio",{origin:"https://app.cal.com"}),Cal.ns["shashini-studio"]("inline",{elementOrSelector:"#my-cal-inline-shashini-studio",config:{layout:"month_view",useSlotsViewOnSmallScreen:"true"},calLink:"indresh-s-eawnbg/shashini-studio"}),Cal.ns["shashini-studio"]("ui",{hideEventTypeDetails:!1,layout:"month_view"})}async function z(){if(!t.user)return showLoginModal();d.innerHTML='<div class="container" style="text-align:center; padding: 100px;"><p>Checking your photography vaults...</p></div>';try{const e=await fetch(`${p}/vaults?mobile=${t.user.mobile}`);t.vaults=await e.json(),d.innerHTML=`
            <div class="container slide-up" style="margin-top: 60px;">
                <div class="section-header" style="flex-direction: column; align-items: center; text-align: center;">
                    <div>
        ...
        `}catch(e){console.error("[Vault Access Error]",e),d.innerHTML=`<div class="container" style="text-align:center; padding: 100px;">
        ...
        </div>`}}window.enterVault=async(e,i)=>{var o;if((!t.vaults||t.vaults.length===0)&&t.user)try{const s=await fetch(`${p}/vaults?mobile=${t.user.mobile}`);t.vaults=await s.json()}catch(s){console.error("Failed to lazy load vaults:",s)}if(t.currentVault=(o=t.vaults)==null?void 0:o.find(s=>s.id&&String(s.id)===String(i)||s.vaultId===e),!t.currentVault){console.error("Vault not found locally:",i),d.innerHTML=`<div class="container" style="text-align:center; padding: 100px;">
        ...
        </div>`;return}d.innerHTML='<div class="container" style="text-align:center; padding: 100px;"><p>Accessing photos...</p></div>';try{const[s,a]=await Promise.all([fetch(`${p}/vaults/selections`).then(n=>n.json()),fetch(`${p}/vaults/${e}/photos`).then(n=>n.json())]);if(a.error)throw new Error(a.error);const l=s.find(n=>n.vaultId===e&&n.mobile===t.user.mobile);if(t.selectedPhotos=new Set(l?l.selections:[]),t.photos=a.photos||[],t.photos.length===0){d.innerHTML=`
            ...
            `;return}E()}catch(s){d.innerHTML=`
            <div class="container" style="text-align:center; padding: 100px;">
                <h3>Access Error</h3>
                <p style="color: #666; margin-top: 10px;">${s.message||"We could not verify your folder."}</p>
                <button class="cta-btn" style="margin-top: 20px;" onclick="state.view='vault'; render();">BACK TO VAULTS</button>
            </div>
        `}};function E(){const e=t.currentVault.status==="locked";if(d.innerHTML=`
        <div class="container slide-up" style="margin-top: 60px;">
            <div class="section-header" style="margin-bottom: 30px; flex-direction: column; align-items: center; text-align: center;">
                <div>
                    <span class="section-subtitle">${e?"🔒 LOCKED":"📸 ACTIVE"} COLLECTION</span>
                    <h2 class="section-title">${t.currentVault.sessionTitle}</h2>
                    <p style="font-size: 12px; color: #999; margin-top: 8px;">
                        ${t.photos.length} photos • ${t.selectedPhotos.size} selected
                        ${e?"":" • Click to select, tap image to preview"}
                    </p>
                </div>
                ${e?'<div style="padding: 12px 24px; background: rgba(231, 76, 60, 0.1); border-radius: 15px; color: #E74C3C; font-weight: 700; font-size: 12px;">SELECTION FINALIZED</div>':`
                    <div style="display: flex; gap: 15px; align-items: center;">
                        <button class="cta-btn" id="save-selection" style="padding: 12px 30px; font-size: 13px; background: rgba(184, 156, 125, 0.2); color: var(--accent); border: 2px solid var(--accent);">
                            💾 SAVE SELECTION (${t.selectedPhotos.size})
                        </button>
                        <button class="cta-btn" id="finalize-selection" style="padding: 12px 30px; font-size: 13px; box-shadow: 0 4px 15px rgba(184, 156, 125, 0.3);">
                            ✓ FINALIZE & SUBMIT (${t.selectedPhotos.size})
                        </button>
                    </div>
                `}
            </div>
            ...
            <div class="vault-grid">
                ${t.photos.map((i,o)=>{const s=t.selectedPhotos.has(i.id);return`
    ...
    `,!e){const i=document.getElementById("save-selection"),o=document.getElementById("finalize-selection");i&&(i.onclick=N),o&&(o.onclick=B)}}window.openLightbox=e=>{const i=document.createElement("div");i.id="lightbox",i.className="lightbox-overlay",i.innerHTML=`
        <div class="lightbox-container">
            ...
            <div class="lightbox-content">
                <img id="lightbox-image" src="${u(t.photos[e].url)}" 
                     referrerpolicy="no-referrer"
                     onerror="if(!this.dataset.backup1) { this.dataset.backup1=true; this.src='https://lh3.googleusercontent.com/d/${t.photos[e].id}'; } else if(!this.dataset.backup2) { this.dataset.backup2=true; this.src='https://drive.google.com/uc?export=view&id=${t.photos[e].id}'; }"
                     alt="Photo ${e+1}">
                <div class="lightbox-info">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span class="lightbox-counter">${e+1} / ${t.photos.length}</span>
                        ${t.currentVault.status!=="locked"?`
                            <button class="lightbox-select-btn ${t.selectedPhotos.has(t.photos[e].id)?"selected":""}" 
                                    onclick="togglePhotoInLightbox(${e})">
                                <span class="select-icon">${t.selectedPhotos.has(t.photos[e].id)?"✓ SELECTED":"+ SELECT"}</span>
                            </button>
                        `:""}
                    </div>
                </div>
    ...
    `,document.body.appendChild(i),document.body.style.overflow="hidden",window.currentLightboxIndex=e,window.lightboxKeyHandler=n=>{n.key==="ArrowLeft"&&navigateLightbox(-1),n.key==="ArrowRight"&&navigateLightbox(1),n.key==="Escape"&&closeLightbox(),n.key===" "&&(n.preventDefault(),togglePhotoInLightbox(window.currentLightboxIndex))},document.addEventListener("keydown",window.lightboxKeyHandler);let o=0,s=0;const a=document.getElementById("lightbox-image");a.addEventListener("touchstart",n=>{o=n.changedTouches[0].screenX}),a.addEventListener("touchend",n=>{s=n.changedTouches[0].screenX,l()});function l(){s<o-50&&navigateLightbox(1),s>o+50&&navigateLightbox(-1)}};window.navigateLightbox=e=>{window.currentLightboxIndex+=e,window.currentLightboxIndex<0&&(window.currentLightboxIndex=t.photos.length-1),window.currentLightboxIndex>=t.photos.length&&(window.currentLightboxIndex=0);const i=document.getElementById("lightbox-image"),o=document.querySelector(".lightbox-counter"),s=document.querySelector(".lightbox-select-btn");i.style.opacity="0",setTimeout(()=>{if(delete i.dataset.backup1,delete i.dataset.backup2,i.src=u(t.photos[window.currentLightboxIndex].url),o.textContent=`${window.currentLightboxIndex+1} / ${t.photos.length}`,s){const a=t.selectedPhotos.has(t.photos[window.currentLightboxIndex].id);s.className=`lightbox-select-btn ${a?"selected":""}`,s.querySelector(".select-icon").textContent=a?"✓ SELECTED":"+ SELECT"}i.style.opacity="1"},200)};window.togglePhotoInLightbox=e=>{window.togglePhoto(t.photos[e].id);const i=document.querySelector(".lightbox-select-btn");if(i){const a=t.selectedPhotos.has(t.photos[e].id);i.className=`lightbox-select-btn ${a?"selected":""}`,i.querySelector(".select-icon").textContent=a?"✓ SELECTED":"+ SELECT"}const o=document.getElementById("save-selection"),s=document.getElementById("finalize-selection");o&&(o.innerHTML=`💾 SAVE SELECTION (${t.selectedPhotos.size})`),s&&(s.innerHTML=`✓ FINALIZE & SUBMIT (${t.selectedPhotos.size})`)};window.closeLightbox=()=>{const e=document.getElementById("lightbox");e&&(e.style.opacity="0",setTimeout(()=>{e.remove(),document.body.style.overflow=""},300)),document.removeEventListener("keydown",window.lightboxKeyHandler),E()};window.togglePhoto=e=>{t.selectedPhotos.has(e)?t.selectedPhotos.delete(e):t.selectedPhotos.add(e),E()};async function N(){if(t.selectedPhotos.size===0)return alert("Please select at least one photo before saving.");const e=document.getElementById("save-selection");e.disabled=!0,e.innerHTML="💾 SAVING...";try{await fetch(`${p}/vaults/select`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({mobile:t.user.mobile,vaultId:t.currentVault.vaultId,vaultName:t.currentVault.sessionTitle,selections:Array.from(t.selectedPhotos)})}),alert("✓ Selection saved! You can continue making changes or finalize when ready.")}catch{alert("Failed to save selection. Please try again.")}finally{e.disabled=!1,e.innerHTML=`💾 SAVE SELECTION (${t.selectedPhotos.size})`}}async function B(){if(t.selectedPhotos.size===0)return alert("Please select at least one photo before finalizing.");const e=`⚠️ IMPORTANT: Finalizing will:

✓ Create a delivery folder: "${t.user.mobile}_selected_pics"
✓ Copy ${t.selectedPhotos.size} selected photo(s) to that folder
✓ Lock this vault (no more changes allowed)

Are you sure you want to finalize your selection?`;if(!confirm(e))return;const i=document.getElementById("finalize-selection");i.disabled=!0,i.innerHTML="⏳ FINALIZING...";try{const o=await fetch(`${p}/vaults/finalize`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({mobile:t.user.mobile,vaultId:t.currentVault.vaultId,selections:Array.from(t.selectedPhotos),customerId:t.user.mobile})}),s=await o.json();if(o.ok&&s.success){const a=`🎉 Selection Finalized Successfully!
...
`:""}
Your vault is now locked. We will process your selected photos.`;alert(a),t.currentVault.status="locked",t.view="vault",m()}else throw new Error(s.error||"Finalization failed")}catch(o){alert(`❌ Finalization Error:

${o.message}

Please contact us for assistance.`),i.disabled=!1,i.innerHTML=`✓ FINALIZE & SUBMIT (${t.selectedPhotos.size})`}}window.renderProfileView=async function(){if(!t.user)return showLoginModal();d.innerHTML='<div class="container" style="text-align:center; padding: 100px;"><p>Loading your profile...</p></div>';try{const[e,i]=await Promise.all([fetch(`${p}/vaults?mobile=${t.user.mobile}`).then(o=>o.json()),fetch(`${b}?action=getProfile&mobile=${t.user.mobile}`).then(o=>o.json())]);i&&i.found?(t.user.name=i.name||t.user.name,t.user.email=i.email||t.user.email,t.user.loginCount=i.loginCount,t.user.lastLogin=i.lastLogin,t.user.collectionSummary=i.collectionSummary):t.user.collectionSummary=e.length+" Sessions",d.innerHTML=`
            <div class="container slide-up" style="margin-top: 60px;">
                ...
                <div class="grid-2" style="gap: 30px;">
                    ...
                    <div class="card" style="padding: 30px; border-radius: 25px; border: 1px solid var(--border-color);">
                        ...
                        <div class="input-group" style="margin-bottom: 20px;">
                            ...
                            <input type="tel" class="login-input" value="${t.user.mobile}" disabled style="opacity: 0.7; padding: 15px; border-radius: 12px; margin-bottom: 0; background: #f5f5f5; font-weight: 700; color: #555;">
                        ...
                        <div class="input-group" style="margin-bottom: 20px;">
                            ...
                            <input type="text" id="profile-name" class="login-input" value="${t.user.name||""}" placeholder="Enter your full name" style="padding: 15px; border-radius: 12px; margin-bottom: 0;">
                        </div>
                        ...
                        <div class="input-group" style="margin-bottom: 30px;">
                            ...
                            <input type="email" id="profile-email" class="login-input" value="${t.user.email||""}" placeholder="e.g. name@example.com" style="padding: 15px; border-radius: 12px; margin-bottom: 0;">
                        </div>
                        ...
                        <div>
                            ...
                            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px; margin-top: 10px;">
                                <div style="padding: 20px; background: rgba(0,0,0,0.02); border-radius: 20px; text-align: center;">
                                    <p style="font-size: 24px; font-weight: 800; color: var(--accent);">${t.user.loginCount||1}</p>
                                    <p style="font-size: 8px; font-weight: 800; color: #999; text-transform: uppercase;">Logins</p>
                                </div>
                                <div style="padding: 20px; background: rgba(0,0,0,0.02); border-radius: 20px; text-align: center;">
                                    <p style="font-size: 24px; font-weight: 800; color: var(--accent);">${e.length}</p>
                            ...
                            <div style="margin-top: 20px; padding: 15px; border-top: 1px solid #eee;">
                                <p style="font-size: 10px; color: #999;">LAST LOGIN: <span style="color: #333; font-weight: 700;">${t.user.lastLogin?new Date(t.user.lastLogin).toLocaleString():"Just now"}</span></p>
                            </div>
                        </div>
        ...
        `}catch(e){if(console.error("Profile load error:",e),!t.user)return showLoginModal();d.innerHTML=`
            <div class="container" style="text-align:center; padding: 100px;">
            ...
            </div>`}};async function L(){d.innerHTML=`
        ...
        <div class="card" style="width: 100%; padding: 50px; border-radius: 40px; box-shadow: 0 40px 100px rgba(0,0,0,0.05);">
            ...
            <div style="display: grid; gap: 20px;">
                <div class="input-group">
                    ...
                    <input type="text" id="msg-name" class="login-input" value="${((e=t.user)==null?void 0:e.name)||""}" style="padding: 18px; border-radius: 15px;" placeholder="e.g. John Doe">
                </div>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div class="input-group">
                        <label class="input-label-premium" style="margin-bottom: 8px;">MOBILE NUMBER</label>
                        <input type="tel" id="msg-mobile" class="login-input" value="${((i=t.user)==null?void 0:i.mobile)||""}" maxlength="10" oninput="this.value = this.value.replace(/\\D/g, '').slice(0, 10)" style="padding: 18px; border-radius: 15px;" placeholder="9876543210">
                    </div>
                    <div class="input-group">
    ...
    `}window.requestCallback=async()=>{const e=document.getElementById("direct-msg-text").value.trim(),i=document.getElementById("msg-name").value.trim(),o=document.getElementById("msg-mobile").value.trim(),s=document.getElementById("msg-email").value.trim();if(!e||!i||!o)return alert("Please fill in your name, mobile number, and message.");if(o.length!==10)return alert("Please enter a valid 10-digit mobile number");const a=document.activeElement;a.disabled=!0,a.innerText="SENDING...";try{const l={action:"add",sender:i,mobile:o,email:s,text:e,timestamp:new Date().toISOString()};(await fetch(`${p}/messages`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)})).ok?(alert("Message sent! We will get back to you soon."),document.getElementById("direct-msg-text").value="",await L()):alert("Failed to send message")}catch{alert("Network error")}finally{a.disabled=!1,a.innerText="SUBMIT ENQUIRY"}};window.showMessageModal=()=>{var i,o,s;const e=document.createElement("div");e.className="modal-overlay",e.style.display="flex",e.innerHTML=`
        <div class="login-modal" style="max-width: 450px; padding: 35px; border-radius: 25px;">
            ...
            <div style="display: grid; gap: 15px;">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div class="input-group">
                        <label style="font-size: 10px; font-weight: 800; color: var(--accent); letter-spacing: 1px; display: block; margin-bottom: 8px;">FULL NAME</label>
                        <input type="text" id="modal-msg-name" class="login-input" placeholder="Your Name" value="${((i=t.user)==null?void 0:i.name)||""}" style="margin-bottom:0; border-radius: 12px;">
                    </div>
                    <div class="input-group">
                        ...
                        <input type="tel" id="modal-msg-mobile" class="login-input" placeholder="Your Mobile" value="${((o=t.user)==null?void 0:o.mobile)||""}" maxlength="10" oninput="this.value = this.value.replace(/D/g, '').slice(0, 10)" style="margin-bottom:0; border-radius: 12px;">
                    </div>
                </div>
                <div class="input-group">
                    ...
                    <input type="email" id="modal-msg-email" class="login-input" placeholder="e.g. john@example.com" value="${((s=t.user)==null?void 0:s.email)||""}" style="margin-bottom:0; border-radius: 12px;">
                </div>
                <div class="input-group">
    ...
    `,document.body.appendChild(e),e.addEventListener("click",a=>{a.target===e&&e.remove()}),document.getElementById("send-modal-msg-btn").onclick=async()=>{const a=document.getElementById("modal-msg-name").value.trim(),l=document.getElementById("modal-msg-mobile").value.trim(),n=document.getElementById("modal-msg-email").value.trim(),r=document.getElementById("modal-msg-text").value.trim();if(!a||!l||!r)return alert("Please fill in Name, Mobile, and Message");if(l.length!==10)return alert("Please enter a valid 10-digit mobile number");const c=document.getElementById("send-modal-msg-btn");c.disabled=!0,c.innerText="SENDING...";try{const g={action:"add",sender:a,mobile:l,email:n,text:r,timestamp:new Date().toISOString()};await fetch(`${p}/messages`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(g)});try{fetch(b,{method:"POST",mode:"no-cors",headers:{"Content-Type":"application/json"},body:JSON.stringify(g)})}catch(f){console.error("Google Sheets sync failed:",f)}c.style.background="#2ECC71",c.innerText="✨ MESSAGE SENT!",setTimeout(()=>e.remove(),1500),t.view==="messages"&&L()}catch{alert("Encountered an error sending message."),c.disabled=!1,c.innerText="SUBMIT ENQUIRY"}}};window.updateProfile=async()=>{const e=document.getElementById("profile-name").value,i=document.getElementById("profile-email").value;if(!e)return alert("Please enter your full name.");const o=document.getElementById("save-profile-btn");o.innerText="SAVING...",o.disabled=!0;try{const s={...t.user,name:e,email:i};await fetch(b,{method:"POST",mode:"no-cors",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"updateProfile",mobile:t.user.mobile,name:e,email:i})}),localStorage.setItem("user",JSON.stringify(s)),t.user=s,m(),alert("✨ Profile updated successfully and synced to Google Sheets!"),window.renderProfileView()}catch(s){console.error("Update failed:",s),alert("Failed to sync changes to cloud.")}finally{o&&(o.innerText="SAVE CHANGES",o.disabled=!1)}};let y=null,h=0,w="";function V(){const e=document.getElementById("hero-media-container");if(!e||!t.cms.hero.slides.length){y&&clearInterval(y),w="";return}const i=JSON.stringify(t.cms.hero);if(i===w)return;w=i,y&&clearInterval(y),e.innerHTML=t.cms.hero.slides.map((s,a)=>{const l=s.type==="video",n=`position:absolute; inset:0; width:100%; height:100%; object-fit:cover; transition: opacity 2s ease-in-out; opacity: ${a===0?1:0}; z-index:1;`;return l?`<video src="${u(s.url)}" class="hero-slide-item" style="${n}" muted loop playsinline></video>`:`<img src="${u(s.url)}" class="hero-slide-item" style="${n}">`}).join("");const o=e.querySelectorAll(".hero-slide-item");if(o[0]&&o[0].tagName==="VIDEO"&&o[0].play().catch(s=>console.warn("Hero video autoplay failed",s)),h=0,o.length>1){const s=(t.cms.hero.interval||5)*1e3;console.log(`[Hero] Carousel started with ${o.length} slides, interval: ${s}ms`),y=setInterval(()=>{const a=(h+1)%o.length;o[h].style.opacity=0,o[h].tagName==="VIDEO"&&setTimeout(()=>o[h].pause(),2e3),o[a].style.opacity=1,o[a].tagName==="VIDEO"&&o[a].play().catch(l=>console.warn("Hero video play failed",l)),h=a},s)}}S();
